...
describe Spotifried::Helpers do
  ...
  describe "#spotify_track" do
    it "should call text_has_spotify_track? and return the result" do
      expect(Spotifried::Helpers).to receive(:extract_spotify_tracks).with("abc").and_return(["xyz"])
  ...
  describe "#extract_spotify_track" do
    ...
    it "should return track id on play urls" do
      expect(Spotifried::Helpers.extract_spotify_tracks("hi https://play.spotify.com/track/#{@track_id} there")).to eq([@track_id])
      expect(Spotifried::Helpers.extract_spotify_tracks("hi http://play.spotify.com/track/#{@track_id} there")).to eq([@track_id])
    end

    it "should return track id on open urls" do
      expect(Spotifried::Helpers.extract_spotify_tracks("hi https://open.spotify.com/track/#{@track_id} there")).to eq([@track_id])
    end

    it "should return track id on spotify uri" do
      expect(Spotifried::Helpers.extract_spotify_tracks("spotify:track:#{@track_id} there")).to eq([@track_id])
    end

    it "should return track id on spotify uri more than one" do
      expect(Spotifried::Helpers.extract_spotify_tracks("spotify:track:#{@track_id} spotify:track:second  there")).to eq([@track_id, "second"])
    end

    it "should return track id on google.com links" do
      expect(Spotifried::Helpers.extract_spotify_tracks("hi https://www.google.com/track/#{@track_id} there")).to eq([])
    end

    it "should return nil on strings without tracks" do
      expect(Spotifried::Helpers.extract_spotify_tracks("hi there")).to eq([])
    end

    it "should return nil on empty strings" do
      expect(Spotifried::Helpers.extract_spotify_tracks("")).to eq([])
    end

    it "should return nil on nil" do
      expect(Spotifried::Helpers.extract_spotify_tracks(nil)).to eq([])
    end
  end
end